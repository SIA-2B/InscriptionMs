...
module.exports = {
    getAllPendingCitations: async (req, res) => {
        try {
            ...
            let pendingCitations = await Citation.find({ student_id: studentId, pending: 1 });
    ...
    getPendingCitationsByCurriculum: async (req, res) => {
        try {
            ...
            let pendingCitations = await Citation.find({ student_id: studentId, curriculum_id: curriculumId, pending: 1 });
            return res.status(200).json({ pendingCitations });
        } catch (error) {
            console.log(error);
            return res.badRequest(new Error('Server Error'));
    ...
    getHistoricCitations: async (req, res) => {
        try {
            let studentId = req.query.student_id;
            let historicCitations = await Citation.find({ student_id: studentId, pending: 0 });
